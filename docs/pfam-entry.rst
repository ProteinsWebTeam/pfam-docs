...
.. figure:: images/pfam_page/overview.png
...
The entry *Overview* tab is displayed by default. It provides information about the type of the Pfam entry, the short 
name and the clan (if the entry belongs to any) are shown at the top, more information about how clans are defined can be 
found in :doc:`Summary </summary>`. Usually, a curated description of the entry is displayed below, with the relevant 
...
2. Only **Reviewed** proteins (from Swiss-Prot - manually curated).

3. Only **Unreviewed** proteins (from TrEMBL - derived from public databases automatically integrated into UniProt).

For each protein, the corresponding protein page in InterPro can be accessed by clicking on the protein accession or name; the taxonomy page can be 
...
can be shown by hovering the mouse over it. You can also export this data in different formats, by clicking on the **Download** button, and customise the page 
settings, by clicking on the wheel icon.

.. figure:: images/pfam_page/proteins.png
  ...
  `A4WYU7 <https://www.ebi.ac.uk/interpro/protein/reviewed/A4WYU7/>`_.
...
.. figure:: images/pfam_page/ida.png
...
.. figure:: images/pfam_page/taxonomy.png
  ...
  The default sunburst chart is shown on the left-hand side, with the mouse hovering over the taxon mammalia. The table view 
  listing the species having proteins belonging to this Pfam entry is displayed on the top right, and the key species matches
...
.. figure:: images/pfam_page/proteome.png
...
.. figure:: images/pfam_page/structures.png
...
1. The first section provides the HMM building command and thresholds used to generate the HMM profile defining the Pfam entry and offers the possibility to download it.

2. The second section shows the HMM logo of the Pfam model, visualised using `Skylign <http://www.skylign.org/>`_. HMM logos are one way of visualising profile HMMs. Logos provide a quick overview of the properties of an HMM in a graphical form.
...
selecting a particular residue in the logo, the probabilities of each amino acid are displayed below the logo.

.. figure:: images/pfam_page/profilehmm.png
  :alt: Example of a Pfam entry with the Signature tab selected.
  :width: 700
  :align: left

  Example of a Pfam entry page (PF02171_) in the tab **Profile HMM** and the 14th residue position in the model sequence selected.
...
.. figure:: images/pfam_page/alphafold.png
...
.. figure:: images/pfam_page/alignment.png
  ...
  bar was dragged to the left to zoom in, allowing to better visualise a specific region of the multiple sequence alignment.
...
Subsequently, Pfam curators set a statistical cut-off, known as a gathering threshold (GA) for an entry, reported hits 
can be found in the :ref:`pfalign` tab on the Pfam entry page. Sequences failing to make a statistical match above this 
threshold are not reported as hits. The threshold is quite conservative, to minimise false positives (although they are 
unavoidable sometimes).
Before every InterPro release, the Pfam model is then run against the whole UniProtKB database and these are the matches 
